...
;;; 
;;; Clojure wurde 2007 von Rich Hickey entwickelt und ist mittlerweile in Version 1.10 verfuegbar.
;; **
...
;;;
;;;   Im Rechnerpool ist Leiningen schon installiert ...
...
;;;
;;;   oder kopieren Sie einfach die Datei von der Vorlesungswebseite dorthin.
;; **
...
;;; 
;;; Alternativ koennen Sie auch einen *notebook* Viewer mittels `lein gorilla` starten oder den integrierten *Nightlight* Editor mittels `lein nightlight`
;;; 
...
(defn sum-to-n [n]
  (loop [i 0, sum 0] ;; Initialisierung
    (if (> i n)
      sum
     (recur (inc i) (+ sum i))))) ;; Werte (von i und sum) fuer naechste Iteration
...
;;; #### Laziness
;;;
;;; Sequenzen in Clojure werden *lazy*, d.h. nur bei Bedarf ausgewertet. Solange die Sequenz nicht angefordert, z.B. durch `println` wird, macht der folgende Code garnichts:
;; **
